...
export default function SpendingAnalytics({ expenses }) {
  ...
  return (
    <div className="bg-white rounded-xl p-4 sm:p-5 border border-gray-100">
      <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-4">Spending Analytics</h2>

      {monthlyExpenses.length === 0 ? (
        <div className="text-center py-6">
          <p className="text-gray-500 text-sm">No spending data yet.</p>
          <p className="text-gray-400 text-xs mt-1">Add expenses to see analytics.</p>
        </div>
      ) : (
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <p className="text-lg font-bold text-gray-900">₹{avgPerDay.toLocaleString()}</p>
            </div>
          </div>

          {highestCategory && (
            <div className="bg-violet-50 rounded-lg p-3">
              <p className="text-xs text-violet-600 mb-1">Highest Spending</p>
